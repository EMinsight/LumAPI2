...
### 1.3 自动补全功能
原生的API中并没有为python提供静态资源以提供自动补全功能，本仓库添加了pyi文件，以提供自动补全功能。  
你也可以通过[生成脚本](../gener_stubs.py)来生成自己的`pyi`文件，然后放入到`LumAPI`目录下或者`LumAPI`打包程序生成的`lumapi.py`同级目录下，即可实现相同效果。

效果实例：
![代码自动补全](explain_pics/autocompletion.png)
