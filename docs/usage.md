# 💻 开发指南
 ...
 - [2. 数据文件.mat和.h5的读取和写入支持](#2-数据文件mat和h5的读取和写入支持)
   - [2.1 matlab数据文件.mat的读取和写入](#21-matlab数据文件mat的读取和写入)
   - [2.2 HDF格式数据文件.h5的读取和写入](#22-hdf格式数据文件h5的读取和写入)
...
### 2.2 HDF格式数据文件.h5的读取和写入
`h5`数据文件为`HDF`格式编码的数据。这种编码能够对数据进行分块压缩储存，从而节省存储空间，适用于大数据文件。同时`matlab`和`origin`等程序也支持读取该文件格式。

*注：本质上`v7.3`的`mat`文件也是这种`HDF`编码，但`matlab`进行自定义了大量的规则导致我们很难完全复制这种规则，这也是为什么`origin`无法读取我们生成的`v7.3`的`mat`文件。不过`matlab`对这种规则的读取采取了比较宽松的条件，我们生成的`v7.3`的`mat`文件能够被`matlab`读取，但无法被采用了严格规则的`origin`读取。*
