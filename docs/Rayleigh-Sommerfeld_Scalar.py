...
def run_rs_validation():
    ...
    for software, mode in tests:
        ...
        E_z_axis = RayleighSommerfeld_Scalar(lamb, x_n, y_n, E_near, 0.0, 0.0, z_scan, mode=mode, software=software)
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'pics/RS_{fn_sg}_{mode}_Z.jpg', dpi=150)
        plt.close()
        
        # 2. XY 焦平面扫描
        E_xy_plane = RayleighSommerfeld_Scalar(lamb, x_n, y_n, E_near, x_f_scan, x_f_scan, actual_f, mode=mode, software=software)
        ...
        plt.colorbar(im)
        plt.tight_layout()
        plt.savefig(f'pics/RS_{fn_sg}_{mode}_XY.jpg', dpi=150)
        plt.close()

        # 3. XZ 全景扫描 (10x2.5 横向沙漏图)
        E_xz_plane = RayleighSommerfeld_Scalar(lamb, x_n, y_n, E_near, x_xz_scan, 0.0, z_scan, mode=mode, software=software)
        ...
        plt.colorbar(im)
        plt.tight_layout()
        plt.savefig(f'pics/RS_{fn_sg}_{mode}_XZ.jpg', dpi=200)
        plt.close()
    ...
    for mode in ['numba', 'vectorized', 'threaded']:
        ...
        plot_diff(diff_xy, f"Diff: {mode.capitalize()} - Common (XY)", 
                  "X (μm)", "Y (μm)", xy_extent, f'pics/RS_diff_{mode}_c_XY.jpg', 
                  is_xy=True, vmax=diff_vmax)
                  
        plot_diff(diff_xz, f"Diff: {mode.capitalize()} - Common (XZ)", 
                  "Z (μm)", "X (μm)", xz_extent, f'pics/RS_diff_{mode}_c_XZ.jpg', 
    ...
    plot_diff(diff_xy_conv, "Diff: Minus - Plus Phase Conv (XY)", 
              "X (μm)", "Y (μm)", xy_extent, 'pics/RS_diff_minus_plus_XY.jpg', 
              is_xy=True, vmax=diff_vmax)
              
    plot_diff(diff_xz_conv, "Diff: Minus - Plus Phase Conv (XZ)", 
              "Z (μm)", "X (μm)", xz_extent, 'pics/RS_diff_minus_plus_XZ.jpg', 
              is_xy=False, vmax=diff_vmax)

if __name__ == "__main__":
    run_rs_validation()
    print("\n所有瑞利-索末菲验证图片与差值分析图已生成完毕！")