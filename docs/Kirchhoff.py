...
def plot_diff(diff_data, title, xlabel, ylabel, extent, filename, is_xy=True, vmax=1e-10):
    ...
    # 增加 vmin=0, vmax=vmax 参数，防止 1e-14 级别的机器误差被渲染成了五颜六色的噪声
...
def run_kirchhoff_validation():
    ...
    for software, mode in tests:
        ...
        E_near = aperture * np.exp(1j * phase)
        
        # 1. Z 轴扫描 (找到真实焦平面 actual_f)
        E_z_axis = Kirchhoff(lamb, x_n, y_n, E_near, 0.0, 0.0, z_scan, mode=mode, software=software)
        I_z_axis = np.abs(E_z_axis[0, 0, :])**2
        ...
        plt.title(f"Z-axis Intensity ({software} | {mode})")
        plt.xlabel("Z (μm)"); plt.ylabel("Intensity")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'pics/Kirchhoff_{fn_sg}_{mode}_Z.jpg', dpi=150)
        plt.close()
        
        # 2. XY 焦平面扫描
        E_xy_plane = Kirchhoff(lamb, x_n, y_n, E_near, x_f_scan, x_f_scan, actual_f, mode=mode, software=software)
        I_xy_plane = np.abs(E_xy_plane[:, :, 0])**2
        ...
        plt.colorbar(im)
        plt.tight_layout()
        plt.savefig(f'pics/Kirchhoff_{fn_sg}_{mode}_XY.jpg', dpi=150)
        plt.close()

        # 3. XZ 全景扫描 (10x2.5 横向沙漏图)
        E_xz_plane = Kirchhoff(lamb, x_n, y_n, E_near, x_xz_scan, 0.0, z_scan, mode=mode, software=software)
        I_xz_plane = np.abs(E_xz_plane[:, 0, :])**2
        ...
        plt.colorbar(im)
        plt.tight_layout()
        plt.savefig(f'pics/Kirchhoff_{fn_sg}_{mode}_XZ.jpg', dpi=200)
        plt.close()
    ...
    for mode in ['numba', 'vectorized', 'threaded']:
        ...
        plot_diff(diff_xy, f"Diff: {mode.capitalize()} - Common (XY)", 
                "X (μm)", "Y (μm)", xy_extent, f'pics/Kirchhoff_diff_{mode}_c_XY.jpg', 
                is_xy=True, vmax=diff_vmax)
                
        plot_diff(diff_xz, f"Diff: {mode.capitalize()} - Common (XZ)", 
                "Z (μm)", "X (μm)", xz_extent, f'pics/Kirchhoff_diff_{mode}_c_XZ.jpg', 
    ...
    plot_diff(diff_xy_conv, "Diff: Minus - Plus Phase Conv (XY)", 
            "X (μm)", "Y (μm)", xy_extent, 'pics/Kirchhoff_diff_minus_plus_XY.jpg', 
            is_xy=True, vmax=diff_vmax)
            
    plot_diff(diff_xz_conv, "Diff: Minus - Plus Phase Conv (XZ)", 
            "Z (μm)", "X (μm)", xz_extent, 'pics/Kirchhoff_diff_minus_plus_XZ.jpg', 
            is_xy=False, vmax=diff_vmax)
